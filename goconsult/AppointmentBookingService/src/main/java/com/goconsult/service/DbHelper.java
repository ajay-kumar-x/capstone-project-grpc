...
public class DbHelper {
        ...
        try(Connection conn=DriverManager.getConnection(URL,USERNAME,PASSWORD)){
            if(conn!=null){
               PreparedStatement ps= conn.prepareStatement("SELECT BOOKING.booking_id, SLOT.slot_id, COUNSELOR.counselor_name, COUNSELOR.contact,COUNSELOR.specialization, date,from_time,to_time,BOOKING.total_amount FROM SLOT  \n" +
                       "  JOIN COUNSELOR ON COUNSELOR.counselor_id=SLOT.counselor_id\n" +
                       "  JOIN BOOKING on SLOT.slot_id=BOOKING.slot_id\n" +
    ...
    public static int rescheduleBookingDetailHelper(Booking.rescheduleDetail request,int fees) {
        int user_id=request.getUserId();
        int booking_id=request.getBookingId();
        int slot_id=request.getSlotId();

        try(Connection conn=DriverManager.getConnection(URL,USERNAME,PASSWORD)){
            if(conn!=null){

                PreparedStatement ps=conn.prepareStatement("UPDATE BOOKING set slot_id=?,user_id=?,total_amount=? where booking_id=?");
                ps.setInt(1,slot_id);
                ps.setInt(2,user_id);
                ps.setInt(3,fees);
